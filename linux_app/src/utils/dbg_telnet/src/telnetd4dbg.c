...
static int get_session_o()
{
    return dup(1);
}

static void session_redirect_o(int fd)
{
    dup2(fd,1);
}
...
static void *the_shell_thread_func(void *arg)
{
    ...
    while (!shell_thread_should_exit)
    {
        ...
        if (ret==1)
        {
            ...
            session_o_fd = get_session_o();
            session_redirect_o(fd_pty_slave);
            proccess_cmd(clean_cmd_line);
            session_redirect_o(session_o_fd);
...
int make_new_session(int new_sock_fd)
{
    ...
    pthread_setname_np(the_shell_thread, "telnet_shell");
    return 0;
}
...
void telnetd4dbg_init(uint16_t port)
{
    ...
    pthread_setname_np(misc_thread, "telnet_misc");
}
